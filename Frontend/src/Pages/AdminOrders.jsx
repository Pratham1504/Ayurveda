...
const OrderList = () => {
    ...
    const handleStatusChange = async (orderId, newStatus) => {
        try {
            await axios.patch(
                `${server}/api/orders/update-status`,
    ...
    const handleRefund = async (orderId) => {
        try {
            ...
            fetchOrders();
            if (selectedOrder?._id === orderId) {
                setSelectedOrder((prev) => ({ ...prev, orderStatus: 'Refund Issued' }));
            }
        } catch (error) {
                        ...
                        <tbody>
                            {sortedOrders.map((order) => (
                                ...
                                >
                                    ...
                                    <td className="p-3">
                                        <span
                                            className={`px-2 py-1 rounded-full text-xs font-semibold ${order.orderStatus.includes('Cancelled') || order.orderStatus.includes('Refund')
